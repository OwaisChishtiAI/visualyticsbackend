...
class Human:
    def __init__(self):
        insert_db = False
        yolo = "/root/vizStuff/model_weights/"
        ...
        self.pattern = self.db["camera01_roi_pattern"]
        if insert_db:
            self.collection.insert_one({"Q"+str(i):0 for i in range(1, 22+1)})
        
        self.collection.update({"_id" : ObjectId("5eb1837084079bfe3ca9caf3")}, {"$set" : {"Q1" : 0, "Q2" : 0, "Q3" : 0, "Q4" : 0, "Q5" : 0, "Q6" : 0, "Q7" : 0, "Q8" : 0, "Q9" : 0, "Q10" : 0, "Q11" : 0, "Q12" : 0, "Q13" : 0, "Q14" : 0, "Q15" : 0, "Q16" : 0, "Q17" : 0, "Q18" : 0, "Q19" : 0, "Q20" : 0, "Q21" : 0, "Q22" : 0 }} ) 
    ...
    def write_roi_to_db(self, roi):
        roi = dict(("Q"+key, val) for key, val in roi.items())
        print("[Info] ROI ", roi)
        self.collection.update({"_id" : ObjectId("5eb1837084079bfe3ca9caf3")}, {"$set" : roi}) 
...
# cap2 = cv2.VideoCapture("images/shelf_video_reverse.mp4")
# human = Human()
# fff = 0
...
#         frame = cv2.resize(frame, (512, 512), interpolation=cv2.INTER_AREA)
#         frame2 = cv2.resize(frame2, (512, 512), interpolation=cv2.INTER_AREA)
#         locs, bbox_cords = human.detect(frame)
#         locs2, bbox_cords2 = human.detect(frame2)
#         for i in range(len(locs)):
#             frame = cv2.putText(frame, str(i), (bbox_cords[i][0], bbox_cords[i][1]), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 2, cv2.LINE_AA)
#         for i in range(len(locs2)):
#             frame2 = cv2.putText(frame2, str(i), (bbox_cords2[i][0], bbox_cords2[i][1]), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 2, cv2.LINE_AA)
#         stack = np.hstack((frame, frame2))
...
# cap = cv2.VideoCapture("images/shelf_video_reverse.mp4")
# human = Human()
# fff = 0
# while cap.isOpened():
#     try:
